 ...
 * Communicates with background.js for auth handling
...
function signIn() {
  ...
  if (!extensionContextValid) {
    console.error('[DEBUG] Extension context invalid');
    showError('Extension context invalid. Please reload the page.');
    return;
  }
  ...
  if (typeof chrome.runtime?.sendMessage !== 'function') {
    ...
    return; // Critical: Prevent retry loop
  }
...
async function createOverlay(whitelistDomains) {
  ...
  const currentArticle = await fetchArticleData(window.location.href);
  if (!currentArticle) {
    console.log('[DEBUG] No matching article found. Extension will not load.');
    return;
  }
  console.log('[DEBUG] Article found:', currentArticle);
  console.log('[DEBUG] Article metadata:', {
    sample_size: currentArticle.sample_size,
    duration: currentArticle.duration,
    effect_size: currentArticle.effect_size,
    design: currentArticle.design,
    rigor: currentArticle.rigor
  });
  ...
  rightSidebar.innerHTML = `
    ...
    <div class="social-discussion" style="margin-top: 40px; padding-top: 40px; border-top: 1px solid #e5e7eb;">
      ...
      <p style="color: #666; font-size: 14px;">Social network integrations coming soon!</p>
